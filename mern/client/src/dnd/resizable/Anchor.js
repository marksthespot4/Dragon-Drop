...
import useStyle from "./AnchorStyle";
...
const Anchor = ({
    ...
    useEffect(() => {
        ...
        const handleResize = event => {
            if (!resizing.current) return;
            const pageX = (isNumber(event.pageX) ? event.pageX : event.touches[0].pageX);
            const pageY = (isNumber(event.pageY) ? event.pageY : event.touches[0].pageY);
        ...
        return () => {
            ...
            cancelClickOnEnd && document.removeEventListener(
                'click', handleClickCancel, {capture: true}
            );
    ...
    const startResize = useCallback(event => {
        ...
        const startY = (
            isNumber(event.pageY) ? event.pageY : event.touches[0].pageY
        );
        setResize({
            startX: startX,
            startY: startY
